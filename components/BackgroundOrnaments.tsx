...
// Hook to detect tab visibility — pauses all wave animations when tab is hidden
...
export default function BackgroundOrnaments() {
  ...
  // Don't animate when tab is not visible to save GPU/battery
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none -z-50 bg-transparent flex items-end opacity-0 dark:opacity-100 transition-opacity duration-500">
      ...
      <motion.div
        ...
        transition={shouldAnimate ? { repeat: Infinity, duration: 20, ease: "linear" } : { duration: 0 }}
      >
      ...
      <motion.div
        ...
        transition={shouldAnimate ? { repeat: Infinity, duration: 25, ease: "linear" } : { duration: 0 }}
      >
        <svg viewBox="0 0 2000 200" preserveAspectRatio="none" className="w-full h-full">
      ...
      <motion.div
        ...
        animate={shouldAnimate ? { x: ["0%", "-50%"] } : undefined}
        transition={shouldAnimate ? { repeat: Infinity, duration: 15, ease: "linear" } : { duration: 0 }}
      >
        <svg viewBox="0 0 2000 200" preserveAspectRatio="none" className="w-full h-full">
      ...
      <motion.div
        ...
        animate={shouldAnimate ? { x: ["-50%", "0%"] } : undefined}
        transition={shouldAnimate ? { repeat: Infinity, duration: 30, ease: "linear" } : { duration: 0 }}
      >
        <svg viewBox="0 0 2000 200" preserveAspectRatio="none" className="w-full h-full">
